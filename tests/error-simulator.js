...
client.on("connect", () => {
  ...
  console.log(
    "🎵 Listen for audio alerts - enable sound in notifications popover!"
  );
  console.log("");
...
function startErrorSimulation() {
  ...
  const sendNextError = () => {
    if (errorIndex >= errorScenarios.length) {
      ...
      console.log(
        "   • Medium/Low errors show notifications + warning/info sounds"
      );
...
function sendDeviceError(scenario) {
  ...
  client.publish(topic, JSON.stringify(errorData), { qos: 1 }, (err) => {
    if (err) {
      ...
      console.log(
        `✅ Error published successfully (${scenario.severity} severity)`
      );
      ...
      if (scenario.severity === "high") {
        console.log(
          "   6. 🚨 HIGH SEVERITY: Modal appears + error sound plays!"
        );
