...
async function checkDeviceStatus(deviceId) {
  try {
    ...
    console.log(
      `   Active Infusion: ${device.activeInfusion ? device.activeInfusion._id : "None"}`
    );
    ...
    if (device.activeInfusion) {
      ...
      console.log(
        `     • Status: ${device.activeInfusion.status || "No status field"}`
      );
    }
...
async function checkInfusionStatus(infusionId) {
  try {
    ...
    console.log(
      `   Device: ${infusion.device ? infusion.device.deviceId : "None"}`
    );
...
async function listAllDevices() {
  try {
    ...
    devices.forEach((device) => {
      console.log(
        `${device.deviceId}: ${device.status} | Active: ${device.activeInfusion ? device.activeInfusion._id : "None"}`
      );
    });
