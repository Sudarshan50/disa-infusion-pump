...
async function runTests() {
  ...
  if (serverRunning) {
    ...
    console.log(
      `   Backend Server: ${serverRunning ? "✅ Running" : "❌ Not Running"}`
    );
    console.log(
      `   Socket.IO: ${socketWorking ? "✅ Working" : "❌ Not Working"}`
    );

    if (serverRunning && socketWorking) {
      console.log("");
      console.log(
        "🎉 All tests passed! Your backend is ready for connections."
      );
      console.log("   You can now test the frontend modal connection.");
    } else if (serverRunning && !socketWorking) {
      console.log("");
      console.log(
        "⚠️ Server is running but Socket.IO is not working properly."
      );
      console.log(
        "   Check if Socket.IO is properly initialized in your backend."
      );
    }
  } else {
    console.log("");
    ...
    console.log(
      "   Please start it with: cd /Users/sudarshan/Documents/infusion-calm && npm start"
    );
  }
