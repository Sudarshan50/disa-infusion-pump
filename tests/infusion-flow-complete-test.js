...
async function testGetDeviceDetails() {
  ...
  try {
    ...
    console.log(
      `   Active Infusion: ${response.data.device.activeInfusion || "None"}`
    );
...
async function testStartInfusion() {
  ...
  try {
    ...
    console.log(
      `   Bolus: ${payload.bolus.enabled ? "enabled" : "disabled"} (${payload.bolus.volumeMl} ml)`
    );

    const response = await apiCall(
      "POST",
      `/start/${config.device.id}`,
      payload
    );
...
async function testWaitForConfirmation(maxWaitTime = 60000) {
  ...
  while (Date.now() - startTime < maxWaitTime) {
    try {
      ...
      if (
        device.data.device.status === "running" &&
        device.data.device.activeInfusion
      ) {
      ...
      console.log(
        `⏳ Still waiting... (${Math.round((Date.now() - startTime) / 1000)}s)`
      );
...
async function testMonitorProgress(durationSeconds = 30) {
  ...
  while (Date.now() - startTime < durationSeconds * 1000) {
    try {
      ...
      console.log(
        `📊 [${elapsed}s] Status: ${device.data.device.status} | Active: ${!!device.data.device.activeInfusion}`
      );
...
async function runCompleteTest() {
  ...
  try {
    ...
    console.log("");
    console.log(
      "💡 Check the frontend dashboard for real-time updates during the test!"
    );
  } catch (error) {
    console.error("");
