...
function handleStartInfusion(payload) {
  ...
  console.log(`   - Planned Volume: ${deviceState.infusion.plannedVolumeMl} ml`);
  console.log(`   - Bolus: ${deviceState.infusion.bolusEnabled ? 'enabled' : 'disabled'} (${deviceState.infusion.bolusVolumeMl} ml)`);
  
  // Handle bolus if enabled
  if (deviceState.infusion.bolusEnabled && deviceState.infusion.bolusVolumeMl > 0) {
    console.log(`💊 Delivering bolus: ${deviceState.infusion.bolusVolumeMl} ml`);
    deviceState.infusion.volumeRemainingMl -= deviceState.infusion.bolusVolumeMl;
  }
...
function publishInfusionConfirmation() {
  ...
  client.publish(topics.infusion, JSON.stringify(message), { qos: 1 }, (err) => {
    if (err) {
...
function publishProgress() {
  ...
  const timeProgress = ((deviceState.infusion.plannedTimeMin - deviceState.infusion.timeRemainingMin) / deviceState.infusion.plannedTimeMin) * 100;
  const volumeProgress = ((deviceState.infusion.plannedVolumeMl - deviceState.infusion.volumeRemainingMl) / deviceState.infusion.plannedVolumeMl) * 100;
  ...
  client.publish(topics.progress, JSON.stringify(message), { qos: 0 }, (err) => {
    if (err) {
      console.error('❌ Failed to publish progress:', err);
    } else {
      console.log(`📈 Progress: ${message.timeRemainingMin.toFixed(1)}min, ${message.volumeRemainingMl.toFixed(1)}ml remaining (${timeProgress.toFixed(1)}% complete)`);
    }
  });
}
...
function startProgressUpdates() {
  ...
  progressInterval = setInterval(() => {
    ...
    if (deviceState.infusion.timeRemainingMin <= 0 || deviceState.infusion.volumeRemainingMl <= 0) {
...
function showDeviceStatus() {
  ...
  if (deviceState.infusion.infusionId) {
    console.log(`  Infusion ID: ${deviceState.infusion.infusionId}`);
    console.log(`  Time Remaining: ${deviceState.infusion.timeRemainingMin.toFixed(1)} min`);
    console.log(`  Volume Remaining: ${deviceState.infusion.volumeRemainingMl.toFixed(1)} ml`);
  }
  console.log('');
}
