...
function sendProgressMessage() {
  ...
  client.publish(topic, JSON.stringify(progressData), { qos: 1 }, (err) => {
    if (err) {
      ...
      console.log(
        `📈 Published progress - Time: ${timeRemaining}min, Volume: ${volumeRemaining}ml`
      );
    }
  });
...
function sendErrorMessage() {
  ...
  client.publish(topic, JSON.stringify(errorData), { qos: 1 }, (err) => {
    if (err) {
      ...
      console.log(
        `🚨 Published error: ${errorData.type} - ${errorData.message}`
      );
    }
  });
}
  ...
  process.stdin.on("readable", () => {
    ...
    if (chunk !== null) {
      ...
      switch (command) {
        ...
        case "help":
          console.log(
            "📋 Available commands: start, progress, error, status, stop"
          );
          break;
        default:
          if (command) {
