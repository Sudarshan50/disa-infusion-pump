...
function startProgressSimulation() {
  ...
  console.log(`🕐 Real-time simulation: ${infusionParams.plannedTimeMin} minutes actual duration`);
  console.log('');
...
function sendProgressUpdate() {
  ...
  const timeRemainingMin = Math.max(0, infusionParams.plannedTimeMin - progressState.elapsedTimeMin);
  const volumeRemainingMl = Math.max(0, infusionParams.plannedVolumeMl - progressState.deliveredVolumeMl);
  
  // Calculate completion percentage
  const timeProgressPercent = (progressState.elapsedTimeMin / infusionParams.plannedTimeMin) * 100;
  const volumeProgressPercent = (progressState.deliveredVolumeMl / infusionParams.plannedVolumeMl) * 100;
  ...
  const progressData = {
    ...
    progressPercent: Math.round(Math.max(timeProgressPercent, volumeProgressPercent) * 100) / 100,
  ...
  client.publish(topic, JSON.stringify(progressData), { qos: 1 }, (err) => {
    if (err) {
      ...
      console.log(`✅ Progress update published (${progressData.progressPercent.toFixed(1)}% complete)`);
      console.log('🔍 Backend should log: "📈 Progress update received for device PUMP_0001"');
      console.log('🔍 Frontend should log: "📈 Received real-time progress update"');
      console.log('');
    }
  });
  
  // Check if infusion is complete
  if (timeRemainingMin <= 0.01 || volumeRemainingMl <= 0.01) { // Small threshold for floating point precision
...
function sendInfusionComplete() {
  ...
  client.publish(progressTopic, JSON.stringify(finalProgressData), { qos: 1 }, (err) => {
    if (err) {
      ...
      setTimeout(() => {
        const completionData = {
          ...
          summary: {
            ...
            efficiency: Math.round((finalProgressData.deliveredVolumeMl / infusionParams.plannedVolumeMl) * 100)
          },
        ...
        console.log(`   Total Time: ${completionData.summary.totalTimeMin} min`);
        console.log(`   Total Volume: ${completionData.summary.totalVolumeMl} ml`);
        console.log(`   Efficiency: ${completionData.summary.efficiency}%`);
        console.log(`   Device Status: ${completionData.deviceStatus}`);
        console.log('');
        
        client.publish(completionTopic, JSON.stringify(completionData), { qos: 1 }, (completionErr) => {
          if (completionErr) {
            console.error('❌ Failed to publish completion notification:', completionErr);
          } else {
            console.log('✅ Completion notification published successfully!');
            ...
            setTimeout(() => {
              ...
              console.log(`   Infusion Complete: ${statusData.infusionComplete}`);
              console.log('');
              
              client.publish(statusTopic, JSON.stringify(statusData), { qos: 1 }, (statusErr) => {
                if (statusErr) {
                  console.error('❌ Failed to publish status update:', statusErr);
                } else {
                  console.log('✅ Status update published successfully!');
                  console.log('');
                  console.log('🎯 Expected Backend Processing:');
                  console.log('   1. Backend receives final progress (100% complete)');
                  console.log('   2. Backend receives completion notification');
                  console.log('   3. Backend updates device status to "healthy"');
                  console.log('   4. Backend updates infusion status to "completed"');
                  console.log('   5. Backend streams completion to Socket.IO');
                  console.log('   6. Frontend receives completion event');
                  console.log('   7. Frontend shows completion notification');
                  console.log('   8. Frontend updates device dashboard to healthy state');
                }
