...
interface DeviceStreamService {
  ...
  connectToDevice: (deviceId: string, options?: { onInfusionRecovered?: (status: DeviceStatus) => void }) => Promise<DeviceStatus>;
  getCurrentDeviceStatus: (deviceId: string) => Promise<DeviceStatus>;
  getDeviceStreamData: (deviceId: string, type: string, count: number) => Promise<ProgressData[]>;
  disconnect: () => void;
...
export const useDeviceStream = (deviceId?: string, options: UseDeviceStreamOptions = {}) => {
  ...
  const [latestStatusUpdate, setLatestStatusUpdate] = useState<StatusUpdate | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (serviceRef.current) {
        ...
        serviceRef.current.removeEventListener('connectionLost', handleConnectionLost);
      }
      window.removeEventListener('deviceInfusionRecovered', handleInfusionRecovered);
    };
  }, [deviceId, baseUrl]);
  ...
  const getStreamData = useCallback(async (type: string = 'progress', count: number = 50) => {
    if (!serviceRef.current || !deviceId) {
      throw new Error('Device not connected');
    }

    return await serviceRef.current.getDeviceStreamData(deviceId, type, count);
  }, [deviceId]);
  ...
  // Auto-connect effect
  useEffect(() => {
    if (autoConnect && deviceId && !connectionStatus.isConnected && !connectionStatus.isConnecting) {
      connectToDevice(deviceId).catch(error => {
        console.error('Auto-connect failed:', error);
      });
    }
  }, [autoConnect, deviceId, connectionStatus.isConnected, connectionStatus.isConnecting, connectToDevice]);
