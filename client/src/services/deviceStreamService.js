...
class DeviceStreamService {
  ...
  async getCurrentDeviceStatus(deviceId) {
    try {
      const response = await fetch(`${this.baseUrl}/api/device/status/${deviceId}`, {
        method: 'GET',
        headers: {
      ...
      console.error(`❌ Failed to get current device status for ${deviceId}:`, error);
      throw error;
    }
  }
  ...
  async getDeviceStreamData(deviceId, type = 'progress', count = 50) {
    try {
      const response = await fetch(`${this.baseUrl}/api/device/stream/${deviceId}?type=${type}&count=${count}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          // Add authorization headers if needed
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (typeof io === 'undefined') {
          console.warn('⚠️ Socket.IO client not available. Install socket.io-client or include the script.');
  ...
  handleReconnection() {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      ...
      this.emitToListeners('connectionLost', { deviceId: this.currentDeviceId });
      return;
    }
  ...
  try {
    await deviceStreamService.recoverDeviceConnection({
      onInfusionRecovered: (status) => {
        ...
        window.dispatchEvent(new CustomEvent('deviceInfusionRecovered', { 
          detail: status 
        }));
      }
    });
